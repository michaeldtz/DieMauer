...
+(NSMutableArray*)loadOverlay:(NSString*)filename:(NSString*)code{
	...
	for(int i = 0; i < [mapPoints count]; i++){
		MapPoint* pnt = [mapPoints objectAtIndex:i];
		if(pnt.type < 3){
			if(pnt.type == 1 && i != 0){
	...
	[mapPoints release], mapPoints = nil;
...
-(NSArray*)createMapData:(NSString*)filename{
	...
	if(data1 != nil){
		...
		if(![parser parse]){
			...
			[textInProgress release], textInProgress = nil;
    ...
    attributes:(NSDictionary *)attributeDict
{
	...
	if(inContent == NO){
		...
		if([[elementName uppercaseString] isEqualToString:@"POINT"]){
			...
			NSString* zoomType = [attributeDict objectForKey:@"zoomtype"];
			if(zoomType != nil)
 ...
 qualifiedName:(NSString *)qName
{
	...
	if(inContent == NO || [[elementName uppercaseString] isEqualToString:@"WEBCONTENT"]){ 
		...
		if([elementsOccured containsObject:elementName]){
			NSString* contentLangCode = [[attributes objectForKey:@"language"] uppercaseString];
			NSArray* preferredLangs = [NSLocale preferredLanguages];
			NSString* usersLangCode   = [[preferredLangs objectAtIndex:0] uppercaseString];	
		...
		if([[elementName uppercaseString] isEqualToString:@"POINT"]){
			...
			[point release],point = nil;
			[elementsOccured release],elementsOccured = nil;
			...
			point.title = [textInProgress copy];
		}else if([[elementName uppercaseString] isEqualToString:@"SUBTITLE"]){
			point.subtitle = [textInProgress copy];
